...
import { InserirPlanoCobrancaViewModel } from '../plano-cobranca.models';
...
@Component({
  ...
  template: `
    <form [formGroup]="form" (ngSubmit)="cadastrar()" class="mx-auto max-w-4xl mt-6">
      <mat-card class="p-6">
        <mat-card-header class="mb-4">
          <mat-card-title>Cadastrar Plano de Cobrança</mat-card-title>
        </mat-card-header>
...
export class CadastrarPlanoCobrancaComponent {
  ...
  public cadastrar() {
    ...
    const request: InserirPlanoCobrancaViewModel = this.form.value;

    this.planoService.cadastrar(request).subscribe({
      next: () => {
        this.notificacaoService.sucesso('Plano de cobrança cadastrado com sucesso!');
